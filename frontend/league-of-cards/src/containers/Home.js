  ...
  generateDeck = (playerObj) => {
    ...
    .then(res => {
      ...
      .then(response => {
        fetch(`http://localhost:3000/api/v1/players/${playerObj.id}`)
        .then(response => response.json())
        // .then(json => {console.log(Object.assign(object, {newCard: json.cards.filter(obj => object.allCards.filter(obj2 => obj.id === obj2.id).length === 0), allCards: "???"}))})
        // .then(json => {object = Object.assign(object, {newCard: json.cards.filter(obj => object.allCards.filter(obj2 => obj.id === obj2.id).length === 0)[0], allCards: json.cards})})
        .then(json => {Object.assign(object, {newCard: json.cards[deckNumber++], allCards: json.cards})})
        .then(response => {
          fetch(`http://localhost:3000/api/v1/deckcards`, {
            method: 'POST',
